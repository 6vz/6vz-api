...
if (!isset ($_GET['img'])) {
	$res = json_decode('{"error": "Błędne zapytanie! Brak zmiennej img!", "status":400, "powered_by":"6vz.pl/api"}');
	header('Content-Type: application/json; charset=utf-8');
	echo json_encode($res, JSON_PRETTY_PRINT);
} else {
	if (empty ($_GET['img'])) {
		$res = json_decode('{"error": "Błędne zapytanie! Pusta zmianna img!", "status":400, "powered_by":"6vz.pl/api"}');
		header('Content-Type: application/json; charset=utf-8');
		echo json_encode($res, JSON_PRETTY_PRINT);
	} else {
			$filePath = $_GET["img"];
